...
export const list: RequestHandler<unknown, unknown, unknown, ListQuery> = (req, res) => {
  ...
  const matched = players
    .filter((p) => {
      ...
      if (maxPriceLakh && p.expectedPriceLakh > Number(maxPriceLakh)) return false;
...
export const getSimilar: RequestHandler<{ id: string }, unknown, unknown, { limit?: string; excludeIpl?: string }> = (
  req,
  res,
) => {

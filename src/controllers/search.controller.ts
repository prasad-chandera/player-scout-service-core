...
function resolveReference({ referencePlayerId, description }: SearchSimilarBody): string | undefined {
  ...
  const aliases = Object.keys(SEARCH_ALIASES).sort((a, b) => b.length - a.length);
...
export const searchSimilar: RequestHandler<unknown, unknown, SearchSimilarBody> = (req, res) => {
  const { referencePlayerId, description, limit = 10, excludeIpl = false } = req.body ?? {};

  const refId = resolveReference({ referencePlayerId, description });
  if (!refId) {
    throw badRequest("UNRESOLVED_QUERY", "Couldn't identify a reference player in the query");
  }

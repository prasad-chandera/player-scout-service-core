...
import type { ExplainComparisonBody, ExplainPlayerBody } from "../types/index.js";

/** 6. POST /api/explain/player */
export const player: RequestHandler<unknown, unknown, ExplainPlayerBody> = async (req, res) => {
  const { playerId, regenerate = false } = req.body ?? {};
  if (!playerId) throw badRequest("MISSING_PLAYER_ID", "Body must include a playerId");
...
export const comparison: RequestHandler<unknown, unknown, ExplainComparisonBody> = async (
  req,
  res,
) => {
  const { playerAId, playerBId, regenerate = false } = req.body ?? {};
  if (!playerAId || !playerBId) {
    throw badRequest("MISSING_PLAYER_ID", "Body must include both playerAId and playerBId");
  }

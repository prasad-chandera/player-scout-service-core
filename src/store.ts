...
import type { Explanation, FeatureMeta, Player, TeamProfile } from "./types/index.js";
...
export const FALLBACK_EXPLANATIONS: Record<string, Explanation> = explanationsFile.explanations;
export const UNDERVALUED_DISCLAIMER: string = explanationsFile.undervaluedDisclaimer;
export const SEARCH_ALIASES: Record<string, string> = explanationsFile.searchAliases;

const playersById = new Map(players.map((p) => [p.id, p]));
const teamsById = new Map(teams.map((t) => [t.id, t]));

export const getPlayer = (id: string): Player | undefined => playersById.get(id);
export const getTeam = (id: string): TeamProfile | undefined => teamsById.get(id);

// Fail loudly at boot rather than shipping a vector the UI will silently mis-render.
function validate(): void {
  for (const p of players) {
    if (!Array.isArray(p.featureVector) || p.featureVector.length !== FEATURES.length) {
      throw new Error(
    ...
    if (p.featureVector.some((v) => typeof v !== "number" || v < 0 || v > 1)) {
      throw new Error(`${p.id}: every featureVector entry must be a number in [0,1]`);
    }
    if (p.phaseStats?.length !== 3) {
      throw new Error(`${p.id}: phaseStats must cover powerplay, middle and death`);
    }
  }
  if (playersById.size !== players.length) throw new Error("Duplicate player id in players.json");
}
validate();

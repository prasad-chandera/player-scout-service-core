...
ok("dataset loaded", all.players.length === 50, `${all.players.length} players`);

let worstDrift = 0;
let worstId = "";
let mismatched = 0;
for (const summary of all.players) {
  const r = await get<ReadinessResponse>(`/api/players/${summary.id}/readiness`);
...
ok("every readiness score matches the player summary", mismatched === 0, `${mismatched} mismatched`);
...
const sim = await get<SimilarSearchResponse>("/api/players/bumrah01/similar?limit=5&excludeIpl=true");
const c = sim.results[0]!.topContributions[0]!;
ok(
  "topContributions reference/candidateValue are strings",
  typeof c.referenceValue === "string" && typeof c.candidateValue === "string",
  JSON.stringify(c),
);
ok(
  "contributions sorted desc, each in [0,1]",
  sim.results[0]!.topContributions.every((x) => x.contribution >= 0 && x.contribution <= 1) &&
...
ok('"find the next bumrah" resolves to Bumrah', search.reference.id === "bumrah01");
ok(
...
ok("similarity spread is meaningful (>15 pts)", spread > 15, `${spread.toFixed(1)} pts`);
ok("nothing rounds to 100%", !search.results.some((r) => Math.round(r.similarity * 100) >= 100));
ok("all similarity values in [0,1]", search.results.every((r) => r.similarity >= 0 && r.similarity <= 1));

const pool = await Promise.all(search.results.map((r) => get<Player>(`/api/players/${r.player.id}`)));
ok("excludeIpl=true excludes IPL players", pool.every((p) => p.competition !== "ipl"));
...
ok(
  ...
  uv.players.every((p, i) => i === 0 || uv.players[i - 1]!.valueGapLakh >= p.valueGapLakh),
);
ok("undervalued ranks are 1..N", uv.players.every((p, i) => p.rank === i + 1));
ok("undervalued carries the calibrated-claim disclaimer", uv.disclaimer.includes("not a market prediction"));
const uvPlayers = await Promise.all(uv.players.map((e) => get<Player>(`/api/players/${e.player.id}`)));
ok("undervalued is SMAT-only", uvPlayers.every((p) => p.competition === "smat"));

// ---------------------------------- teams ----------------------------------

const teams = await get<{ teams: TeamProfile[] }>("/api/teams");
ok("10 teams, each with a `short`", teams.teams.length === 10 && teams.teams.every((t) => Boolean(t.short)));
...
ok("top fitScore is normalised to 100", fit.recommendations[0]!.fitScore === 100, String(fit.recommendations[0]!.fitScore));
ok("team fit respects RCB's 800L budget", fit.recommendations.every((r) => r.player.expectedPriceLakh <= 800));

// ---------------------------------- meta -----------------------------------

const meta = await get<{ version: number; features: FeatureMeta[] }>("/api/meta/features");
ok(
...
const exp = await post<{ explanation?: { summary?: string; weaknesses?: string[] } }>(
  "/api/explain/player",
  { playerId: "arjun-kumar" },
);
ok("explain/player returns an explanation", Boolean(exp.explanation?.summary));
ok("explain has non-empty weaknesses (grounding rule 6)", (exp.explanation?.weaknesses?.length ?? 0) > 0);

const cmp = await post<{ explanation?: { verdict?: string; rows?: unknown[] } }>(
  "/api/explain/comparison",
  { playerAId: "bumrah01", playerBId: "arjun-kumar" },
);
ok("explain/comparison returns verdict + rows", Boolean(cmp.explanation?.verdict) && Array.isArray(cmp.explanation?.rows));

// ---------------------------------- errors ---------------------------------

ok(
  "unknown player -> 404 PLAYER_NOT_FOUND",
  (await get<ErrorResponse>("/api/players/nonexistent")).error?.code === "PLAYER_NOT_FOUND",
);
ok(
  "unknown team -> 404 TEAM_NOT_FOUND",
  (await post<ErrorResponse>("/api/teams/nope/fit", {})).error?.code === "TEAM_NOT_FOUND",
);
ok(
  "unresolvable query -> 400 UNRESOLVED_QUERY",
  (await post<ErrorResponse>("/api/search/similar", { description: "find me a wizard" })).error?.code ===
    "UNRESOLVED_QUERY",
);
ok(
  "explain with no playerId -> 400 MISSING_PLAYER_ID",
  (await post<ErrorResponse>("/api/explain/player", {})).error?.code === "MISSING_PLAYER_ID",
);
ok(

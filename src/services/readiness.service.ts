...
import type { Player, ReadinessBreakdownRow, ReadinessResponse, Role } from "../types/index.js";
...
export function readinessFor(player: Player): ReadinessResponse {
  ...
  if (Math.abs(drift) > 1e-9 && breakdown[0]) {
    breakdown[0].contribution = Math.round((breakdown[0].contribution + drift) * 10) / 10;
  }

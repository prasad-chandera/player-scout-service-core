...
import type { Player, Role, TeamFitRecommendation, TeamNeed, TeamProfile } from "../types/index.js";
...
export function fitFor(
  ...
  const scored = players
    ...
    .map((p) => {
      let best: { fit: number; need: TeamNeed } = { fit: 0, need: team.needs[0] as TeamNeed };
      for (const need of team.needs) {
        const spec = NEED_FEATURES[need.role];
        if (!spec || !spec.roles.includes(p.role)) continue;
        const mean =
          spec.indexes.reduce((sum, i) => sum + (p.featureVector[i] as number), 0) /
          spec.indexes.length;

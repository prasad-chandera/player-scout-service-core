...
import { CACHE_DIR, FALLBACK_EXPLANATIONS, players as allPlayers } from "../store.js";
import { MODEL_VERSION, readinessFor, topContributors } from "./readiness.service.js";
import { similarTo } from "./similarity.service.js";
import type { ComparisonExplanation, Explanation, Player } from "../types/index.js";
...
const EXPLANATION_SCHEMA: JsonSchema = {
  ...
  properties: {
    summary: {
      type: "string",
      description: "2-3 sentences. Lead with the defining skill and the number that proves it.",
    },
    strengths: {
      type: "array",
      items: { type: "string" },
    ...
    weaknesses: {
      type: "array",
      items: { type: "string" },
      description: "1-3 weaknesses, each quoting a supplied number. Never empty.",
    },
    comparablePlayers: {
      type: "array",
      items: {
...
interface PlayerPayload {
  player: { name: string; role: string; age: number; competition: string; matches: number };
  stats: Record<string, number>;
  readiness: { score: number; topContributors: string[] };
  similarPlayers: { name: string; similarity: number; sharedStrengths: string[] }[];
...
function percentile(
  ...
  const values = pool.map((p) => p.rawStats[key]).filter((v): v is number => typeof v === "number");
...
function statsWithPercentiles(player: Player): Record<string, number> {
  ...
  for (const [key, value] of Object.entries(player.rawStats)) {
    ...
    const higherIsBetter =
      !LOWER_IS_BETTER.has(key) && !(key === "dotBallPct" && player.role === "batter");
...
async function readCache<T>(file: string): Promise<T | null> {
  try {
    return JSON.parse(await fs.readFile(path.join(CACHE_DIR, file), "utf8")) as T;
  } catch {
    return null;
  }
}
...
async function callClaude<T>(system: string, payload: unknown, schema: JsonSchema): Promise<T> {
...
export async function explainPlayer(
  ...
  if (!hasKey()) {
    return {
      explanation: FALLBACK_EXPLANATIONS[player.id] ?? genericExplanation(player),
      cached: true,
    };
  }

  try {
    const explanation = await callClaude<Explanation>(SYSTEM_PROMPT, payload, EXPLANATION_SCHEMA);
...
export async function explainComparison(
  ...
  const payload = {
    ...
    similarity: Number(similarTo(a, [b], { limit: 1 })[0]?.similarity.toFixed(2) ?? 0),
  };
  ...
  try {
    ...
    const explanation = await callClaude<ComparisonExplanation>(system, payload, COMPARISON_SCHEMA);
    await writeCache(file, explanation);
    return { explanation, cached: false };
  } catch (err) {
    console.error(

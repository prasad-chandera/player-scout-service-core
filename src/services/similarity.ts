...
const POOL_MEAN: number[] = FEATURES.map(
	(_, i) =>
		allPlayers.reduce((sum, p) => sum + (p.featureVector[i] as number), 0) /
		allPlayers.length
)

export const centre = (v: number[]): number[] =>
	v.map((x, i) => x - (POOL_MEAN[i] as number))
...
export function contributions(a: number[], b: number[]): ContributionTerm[] {
	...
	return terms.map((t, index) => ({
		index,
		contribution: Math.max(0, t) / positive
	}))
}
	...
	switch (featureKey) {
		...
		default: {
			if (featureKey === 'pressure')
				return `${(p.skillGroups.pressure * 10).toFixed(1)}/10`
			if (featureKey === 'consistency')
				return `${(p.skillGroups.consistency * 10).toFixed(1)}/10`
...
function inPool(reference: Player, p: Player, excludeIpl: boolean): boolean {
	...
	return (
		p.role === reference.role ||
		p.role === 'allrounder' ||
		reference.role === 'allrounder'
	)
}
